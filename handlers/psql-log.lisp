...
(defhandler psql-log (channel message)
  ...
  (unless postmodern:*database*
    (let ((database (conf-value 'database)))
     (postmodern:connect-toplevel (cdr (assoc 'name database))
                                  (cdr (assoc 'user database))
                                  (cdr (assoc 'password database))
                                  (cdr (assoc 'host database)))))
